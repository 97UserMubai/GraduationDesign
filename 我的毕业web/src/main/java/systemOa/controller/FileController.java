...
public class FileController extends HttpServlet {
    ...
    @Autowired
    @Qualifier("fileBeanService")
    private IFileBeanService iFileBeanService;

    public IFileBeanService getiFileBeanService() {
        return iFileBeanService;
    }

    public void setiFileBeanService(IFileBeanService iFileBeanService) {
        this.iFileBeanService = iFileBeanService;
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        //文件日志登记的文件人和部门
        String filePerson = employee.getName();
        String fileDepartment = employee.getDepartment();
        String fileName;
        String fileSource;
        //得到上传文件的保存目录，将上传的文件存放于WEB-INF目录下，不允许外界直接访问，保证上传文件的安全

        System.out.println("##########doGet的测试");
        System.out.println(request.toString());
        String savePath = "C:/Users/木白/Desktop/毕业设计GitHub仓库/GraduationDesign/我的毕业web/src/main/webapp/WEB-INF/upload";
        System.out.println(savePath);
        //上传时生成的临时文件保存目录
        System.out.println("######文件存储路径获取测试");
        String tempPath = "C:/Users/木白/Desktop/毕业设计GitHub仓库/GraduationDesign/我的毕业web/src/main/webapp/WEB-INF/temp";
        ...
        try{
            ...
            for(FileItem item : list){
                ...
                if(item.isFormField()){
                    ...
                    iOperationLogService.insertLog(new OperationLog(filePerson,date,opeAction,opeAll));
                    System.out.println("测试2##############");
                    //删除处理文件上传时生成的临时文件
                    //item.delete();
                    message = "文件上传成功！";
   ...
   //打散存储
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response,HttpSession session)
            throws ServletException, IOException {
        System.out.println("#######doPost的测试");
        doGet(request, response,session);
    }

    public void doGetDownload(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        //得到要下载的文件名
        String fileName = request.getParameter("filename");  //23239283-92489-阿凡达.avi
        fileName = new String(fileName.getBytes("iso8859-1"),"UTF-8");
        //上传的文件都是保存在/WEB-INF/upload目录下的子目录当中
        String fileSaveRootPath="C:/Users/木白/Desktop/毕业设计GitHub仓库/GraduationDesign/我的毕业web/src/main/webapp/WEB-INF/upload";
        //通过文件名找出文件的所在目录
    ...
    public void doPostDownload(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
